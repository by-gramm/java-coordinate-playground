...
- [x] 좌표 정보 입력받기
  - [x] 좌표 정보는 괄호로 둘러쌓여 있으며, 쉼표로 X, Y값을 구분한다.
  - [x] 좌표를 여러 개 입력하는 경우, 좌표값 사이는 '-' 문자로 구분한다.
  - [x] 좌표 정보 예외 처리
    - [x] 입력값 형식이 유효한 좌표 정보여야 한다.
    - [x] X, Y 좌표 모두 0 이상 24 이하의 **정수**여야 한다.
    - [x] 좌표값은 2개 이상 4개 이하여야 한다.
...
- [x] 입력받은 좌표 정보 저장하기
  ...
  - [x] 좌표값을 4개 입력한 경우, 네 점을 연결하는 직사각형으로 가정한다.
    ...
    - [x] 직사각형의 넓이를 계산한다.
- [x] 좌표 정보 출력하기
  - [x] 0 ~ 24 사이의 값을 나타낼 수 있는 X축과 Y축을 출력한다.
  - [x] 좌표들을 출력한다.
  - [x] 좌표가 2개면 직선의 거리를, 3 or 4개면 도형의 넓이를 출력한다.
